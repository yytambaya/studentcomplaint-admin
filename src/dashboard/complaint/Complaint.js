...
import { validateTitle, validateText, validateEmail, validatePassword, validateNumber, validateName, validatePhoneNumber, validateDescription, validateRequiredDescription } from "../../services/validators";
...
const Complaint = ({setPage, setLastPage, lastPage, complaint}) => {
    ...
    const [tempMarkedAs, setTempMarkedAs] = useState(false)
    const [tempReply, setTempReply] = useState(false)
    ...
    const [error, setError] = useState([{field: "reply", msg:""}, {field: "email", msg:""}, {field: "phoneNumber", msg: ""},{field: "password", msg:""}, {field: "status", msg:""} ]);
    ...
    const markComplaintAsResolved = async (complaint_id) => {
        //alert("Title: " + title + " Text: " + text + " Checked: " + publish);
        var accessToken = localStorage.getItem('jwt_token');
        ...
        if(at_val){
            //alert("going")
            const url = `${getAPIBaseURL()}/v1/admin/complaint/tracking-status`;
            const api_key = '@!8(T#7<R:I#:F1#r!>BW/!';
            const headers = {'x-access-key': api_key, 'x-access-token': accessToken}
    ...
    const replyToComplaint = async (complaint_id) => {
        //alert("Title: " + title + " Text: " + text + " Checked: " + publish);
        var accessToken = localStorage.getItem('jwt_token');
        var at_val = accessToken == ""? false : true; 
        if(accessToken == "") setGenError("Unauthorized complaint. Login again!"); 
        var reply_val = validateRequiredDescription(reply).error == "" ? true: false;
        setError([...error, error.find(item => item.field == "reply").msg = validateRequiredDescription(reply).result])
        
        
        if(at_val && reply_val){
            //alert("going")
            const url = `${getAPIBaseURL()}/v1/admin/complaint/reply`;
            const api_key = '@!8(T#7<R:I#:F1#r!>BW/!';
            const headers = {'x-access-key': api_key, 'x-access-token': accessToken}
            const data = {id: complaint_id, reply};
    ...
    return(
        ...
        {/*<Notification message={"A new complaint is successfully created!"}/>*/}
            {showReplyButton && <div class=" backdrop-blur-sm overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0  z-50 md:inset-0 h-modal md:h-full justify-center items-center flex" aria-modal="true" role="dialog">
                <div className=" shadow-2xl flex flex-col fixed top-40 bg-white space-y-4 px-4 py-2 text-xl">
                    <h1 className="text-2xl font-mono">Reply to this complaint</h1>
                    <div class="mb-6">
                        <label for="description" class="block mb-2 text-xl text-black">Text</label>
                        <textarea onChange={(e) => setReply(e.target.value)} id="title" class="text-lg outline-none focus:border-gray-400 border-2  w-full px-4 py-2" placeholder="This is a very ..." required=""/>
                        {error.find(item => item.field == "reply").msg ? <p class="text-red-500 text-sm font-semibold mt-1">{error.find(item => item.field == "reply").msg }</p>: null}
                    </div>
                    <div className="flex justify-between">
                        <button onClick={() => replyToComplaint(complaint._id)} type="submit" class="flex text-white bg-red-700 px-6 py-2 mb-8">{"Reply"}</button>
                        <button onClick={() => setShowReplyButton(false)} type="" class="flex text-white bg-gray-300 px-6 py-2 mb-8">{"Cancel"}</button>
                    </div>
                </div>
            </div>}
        ...
        <div class="mx-5 md:mx-20 mt-10">
            ...
            <div>
                ...
                {tempReply && <div className='mt-4 ml-16'>
                    <p className='text-lg font-semibold'>Reply</p>
                    {reply}
                </div>}
            <div className='flex space-x-4 mt-8'>
                <p className=' font-medium'>Complain status: </p>
                <p className=' font-bold'>{tempMarkedAs ? "resolved" : complaint.trackingStatus}</p>
            </div>
            <div className='flex flex-row space-x-8 mt-16'>
                {complaint?.trackingStatus == "resolved" ? null : <button onClick={() => markComplaintAsResolved(complaint._id)} type="submit" class="flex text-white bg-green-700 px-6 py-2 mb-8">{"Mark as resolved"}</button>}
                {complaint?.reply? null : <button onClick={() => setShowReplyButton(true)} type="submit" class="flex text-white bg-gray-700 px-6 py-2 mb-8">{"Reply"}</button>}
